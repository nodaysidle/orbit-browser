...
**Process:** Full code audit → detailed design document → approved plan execution → additional features → documented build/install workflow
...
- No new Rust dependencies in the original UI polish wave; current download support uses `reqwest` 0.12 with `rustls-tls`
...
- Final end-to-end visual QA checklist execution mindset (clean builds + visual verification where available).
...
The repository now documents a conservative local build/install workflow:

1. Run `npm run tauri build` or `bash scripts/build-mac.sh` to produce the macOS app bundle.
2. When an install is explicitly desired, use `bash scripts/install-app.sh` or copy the built bundle to `/Applications/Orbit.app`.
3. Treat `/Applications` installs and release packaging as explicit operator actions, not implicit documentation claims.
...
- New Rust dependencies in the original UI polish wave
