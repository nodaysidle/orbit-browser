...
The current state is a solid, shippable foundation with recent session-restore hardening (README 2026-05-28) and clean `npm run check` results (25 JS tests + 60 Rust tests + clippy clean + production Vite build all passing). High-impact polish remains available entirely within existing files and patterns.
...
Orbit ships as a single signed `.app` bundle. No server flags.
...
- No new third-party JS or Rust crates; any advanced animation stays in CSS keyframes + existing cubic-bezier.
...
- No new Rust crates; no CSP changes in `tauri.conf.json`; unstable feature untouched; `build.rs` untouched.
