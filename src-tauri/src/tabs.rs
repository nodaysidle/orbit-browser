// Lock ordering: when acquiring multiple BrowserState locks, always acquire in
// this order to prevent deadlocks: tabs -> tab_order -> active_tab
...
fn save_current_session_impl(app: &AppHandle, state: &BrowserState) {
    let db = app.state::<Db>();
...
fn notify_session_dirty(state: &BrowserState) {
    state.session_notify.notify_one();
}

pub fn spawn_session_debouncer(app: AppHandle) {
            ...
            .build()
        {
            ...
            Err(err) => {
                report_error(format_args!(
                    "failed to build session debouncer runtime: {err}"
                ));
                return;
            }
        };
        let notify = app.state::<BrowserState>().inner().session_notify.clone();
        rt.block_on(async move {
            loop {
                notify.notified().await;
                // Debounce: wait 500ms, resetting if another notification arrives
                loop {
                    tokio::select! {
                        _ = tokio::time::sleep(std::time::Duration::from_millis(500)) => {
                            let state = app.state::<BrowserState>();
                            save_current_session_impl(&app, state.inner());
                            break;
                        }
                        _ = notify.notified() => {
                            // Another request arrived — reset the timer
                            continue;
                        }
                    }
                }
...
pub async fn create_tab(
    ...
    if !is_blank {
        if let Err(err) = create_webview_for_tab(&app, &state, &id, &clean_url, make_active).await {
            ...
            lock_state(&state.tab_order, "tab_order")?.retain(|order_id| order_id != &id);
            if make_active {
    ...
    // Persist session
    notify_session_dirty(state.inner());
                ...
                .on_page_load(move |_wv, payload| {
                    ...
                    match payload.event() {
                        ...
                        PageLoadEvent::Finished => {
                            ...
                            notify_session_dirty(browser_state.inner());
...
pub async fn close_tab(
    ...
    // Lock order: tabs -> tab_order -> active_tab (canonical, prevents deadlocks)
...
mod tests {
    ...
    #[test]
    fn test_create_tab_error_cleans_tab_order() {
        let state = BrowserState::new();
        // Simulate the cleanup sequence when create_webview_for_tab fails
        lock_state(&state.tabs, "tabs")
            .unwrap()
            .insert("tx".into(), test_tab_data("tx", "https://example.com"));
        lock_state(&state.tab_order, "tab_order")
            .unwrap()
            .push("tx".to_string());

        // Execute cleanup: remove from tabs AND tab_order
        lock_state(&state.tabs, "tabs").unwrap().remove("tx");
        lock_state(&state.tab_order, "tab_order")
            .unwrap()
            .retain(|order_id| order_id != "tx");

        // Verify the ID is absent from both collections
        assert!(!tab_exists(&state, "tx"));
        assert!(lock_state(&state.tab_order, "tab_order")
            .unwrap()
            .iter()
            .all(|id| id != "tx"));
    }
