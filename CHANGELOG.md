...
- Build/install workflow documented for producing the macOS app bundle and copying it to `/Applications/Orbit.app` when explicitly run.
- All work strictly followed project constraints: Vanilla JavaScript, exact frontend module structure preserved, WKWebView child webviews, locked CSP, no modifications to `build.rs`. Current download support uses `reqwest` 0.12 with `rustls-tls`.
...
**Process & Quality:**
- Full `npm run check` validation (26 JS tests + 67 Rust tests + format + clippy `-D warnings` + production Vite build) after every significant change batch.
...
- **Build:** Release app bundle produced by the Tauri build pipeline; signing/notarization/release packaging is not claimed here.
...
- Limited the default macOS bundle target to the macOS app bundle used by the installer.
